...
public class MessageListenerExample extends ListenerAdapter
{
	...
	private static DBO dbo; //database object
	...
	public static void main(String[] args)
	{
		...
		try
		{
			//			dbo = new DBO(); //initialize DBO

			JDA jda = new JDABuilder(AccountType.BOT)
					.setToken("MzEyMjcxMDYyODUzNDg0NTQ3.C_YqfA.Y5fy5m4NCk6BD9BzxZFONBYV1VI")  //The token of the account that is logging in.
	...
	public void onMessageReceived(MessageReceivedEvent event)
	{
		...
		// This is a command to automatically convert timezones.
		else if (msg.startsWith("!time"))
		{
			...
			channel.sendMessage("This is a placeholder to test output. The current time is: ").queue();
			channel.sendMessage(localTime.getTime().toString()).queue();
			channel.sendMessage("This message was sent from: " + message.getGuild().getId()).queue();
			channel.sendMessage("This message was: '" + message.getContent() + "'").queue();
			...
			if(invalid)
			{
				channel.sendMessage("Must be in format: '!time hh:mm timezone'").queue();
			}
		}
